...
export function PowerRankingsPage({ teams, standings, schedule, userTeamId, onViewTeam, onViewPlayer }: PowerRankingsPageProps) {
  ...
  const regionNames: Record<Region | 'global', string> = {
    global: 'Global',
    americas: 'Americas',
    emea: 'EMEA',
    pacific: 'Pacific',
  ...
  const filteredRankings = selectedRegion === 'global' 
    ? rankings 
    ...
    <div className="power-rankings-page">
      ...
      <div className="pr-header">
        <div className="pr-title-row">
          ...
          <span className="pr-count">{teams.length} teams • {allPlayers.length} players</span>
        </div>
        
        {/* Main Tab Navigation */}
        <div className="pr-main-tabs">
          <button 
            className={`pr-main-tab ${activeTab === 'teams' ? 'active' : ''}`}
          ...
          <button 
            className={`pr-main-tab ${activeTab === 'players' ? 'active' : ''}`}
      ...
      <div className="pr-subnav">
        ...
        <div className="pr-region-tabs">
          <button 
            className={`pr-region-tab ${selectedRegion === 'global' ? 'active' : ''}`}
        ...
        <div className="pr-content">
          ...
          <div className="panel">
            <div className="panel-header">
              {selectedRegion === 'global' ? '🌍 Global Rankings' : `${regionNames[selectedRegion]} Rankings`}
              <span className="panel-header-sub">{filteredRankings.length} teams</span>
            </div>
              ...
              <table className="power-rankings-table">
                <thead>
                  <tr>
                    ...
                    <th className="rating-col">OVR</th>
                    <th className="score-col">{rankingMode === 'power' ? 'PWR' : 'STR'}</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredRankings.map((r, idx) => {
                    const displayRank = selectedRegion === 'global' ? r.rank : idx + 1;
                    ...
                    return (
                      <tr key={r.team.id} className={isUser ? 'user-row' : ''}>
                        ...
                        <td className="map-col">{r.standing.mapWins}-{r.standing.mapLosses}</td>
                        <td className="form-col">
                          <div className="form-indicator">
                            {r.recentForm.length > 0 ? (
                              r.recentForm.slice(-5).map((result, i) => (
                                <span key={i} className={`form-dot ${result === 'W' ? 'win' : 'loss'}`}>
                                  {result}
                                </span>
                              ))
                            ) : (
                        ...
                        <td className="score-col">
                          <span className="power-score">
                            {rankingMode === 'power' ? Math.round(r.powerScore) : r.lineupStrength}
                          </span>
                        </td>
                      </tr>
                    );
      ...
      {activeTab === 'players' && (
        <div className="pr-content">
          {/* Stats Legend */}
          <div className="pr-stats-legend">
            <div className="legend-item"><span className="legend-abbr">ACS</span><span className="legend-desc">Average Combat Score (damage, kills, assists per round)</span></div>
            <div className="legend-item"><span className="legend-abbr">K/D</span><span className="legend-desc">Kill/Death Ratio</span></div>
            <div className="legend-item"><span className="legend-abbr">FK/Map</span><span className="legend-desc">First Kills per Map (opening duels won)</span></div>
            <div className="legend-item"><span className="legend-abbr">WIN%</span><span className="legend-desc">Match Win Rate</span></div>
            <div className="legend-item"><span className="legend-abbr">RTG</span><span className="legend-desc">Performance Rating (weighted combination of all stats)</span></div>
          </div>

          {/* MVP Spotlight + Top 10 */}
          <div className="pr-players-grid">
            {/* MVP Card */}
            {mvpPlayer && (
              <div className="pr-mvp-card" onClick={() => onViewPlayer(mvpPlayer.player.id)}>
                ...
                <div className="mvp-main">
                  ...
                  <div className="mvp-info">
                    ...
                    <div className="mvp-meta">
                      <span className="mvp-role" style={{ background: roleColors[mvpPlayer.player.role] }}>
                        {mvpPlayer.player.role.toUpperCase()}
                      </span>
                      <span className="mvp-region">{regionNames[mvpPlayer.team.region]}</span>
                      {mvpPlayer.hasStats && <span className="mvp-maps">{mvpPlayer.breakdown.mapsPlayed} maps</span>}
                    </div>
                  </div>
                  <div className="mvp-rating">
                    <div className="mvp-rating-value">{Math.round(mvpPlayer.performanceRating)}</div>
                    <div className="mvp-rating-label">RTG</div>
                  </div>
                </div>
                <div className="mvp-stats">
                  {mvpPlayer.hasStats ? (
                    <>
                      <div className="mvp-stat"><span className="stat-value">{Math.round(mvpPlayer.breakdown.acs)}</span><span className="stat-label">ACS</span></div>
                      <div className="mvp-stat"><span className="stat-value">{mvpPlayer.breakdown.kd.toFixed(2)}</span><span className="stat-label">K/D</span></div>
                      <div className="mvp-stat"><span className="stat-value">{mvpPlayer.breakdown.fkPerMap.toFixed(1)}</span><span className="stat-label">FK/Map</span></div>
                      <div className="mvp-stat"><span className="stat-value">{Math.round(mvpPlayer.breakdown.winRate)}%</span><span className="stat-label">WIN%</span></div>
                    </>
                  ) : (
                    <>
                      <div className="mvp-stat"><span className="stat-value">{mvpPlayer.player.ratings.aim}</span><span className="stat-label">AIM</span></div>
                      <div className="mvp-stat"><span className="stat-value">{mvpPlayer.player.ratings.gameSense}</span><span className="stat-label">IQ</span></div>
                      <div className="mvp-stat"><span className="stat-value">{mvpPlayer.player.ratings.clutchFactor}</span><span className="stat-label">CLUTCH</span></div>
                      <div className="mvp-stat"><span className="stat-value">{mvpPlayer.player.overall}</span><span className="stat-label">OVR</span></div>
                    </>
                  )}
                </div>
              </div>
            ...
            {/* Top 10 Table */}
            <div className="panel pr-top-players">
              <div className="panel-header">
                🌍 Top 10 Players
                <span className="panel-header-sub">by Performance</span>
              </div>
              <div className="panel-body" style={{ padding: 0 }}>
          ...
          <div className="pr-role-leaders">
            <h3 className="section-title">Best at Every Position</h3>
                ...
                return (
                  ...
                  >
                    <div className="role-leader-header" style={{ background: roleColors[role] }}>
                      <span>{roleNames[role].toUpperCase()}</span>
                      <span>👑</span>
                    </div>
                      ...
                      <div className="role-leader-info">
                        ...
                        {roleLeader.hasStats && (
                          <div className="role-leader-stats">
                            {Math.round(roleLeader.breakdown.acs)} ACS • {roleLeader.breakdown.kd.toFixed(2)} K/D
                          </div>
                        )}
                      </div>
                      <div className="role-leader-rating">
                        {Math.round(roleLeader.performanceRating)}
                        <span>RTG</span>
                      </div>
                    </div>
                    <div className="role-runner-ups">
                      {topByRole[role].slice(1, 4).map((p, idx) => (
                        ...
                        >
                          ...
                          <span className="runner-rtg">{Math.round(p.performanceRating)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
